...
- [ ] **4b Lenses:** collapse the session to one dimension — diffs only · bash only · errors only · prompts only. Cheap: `kind` is already a column.
- [ ] **4e In-session find + sticky "you are here":** session-scoped find with next/prev (FTS `session=` filter on the search endpoint) and a breadcrumb of the current turn. Table stakes — must be flawless.
- [ ] **4c Jump-to-error markers:** "N errors in this session →" from `is_error`, as jump points.
