...
- `src/export/markdown.ts` — session → markdown serializer; dependency-free (minimal ±line diffs, no diff lib in the server). Used by `turnlog export <id>` and `GET /api/sessions/:id/export`
