...
- `src/export/` — session serializers, all dependency-free: `markdown.ts` (minimal ±line diffs, no diff lib in the server), `html.ts` (self-contained styled page, everything inline, content escaped — logs are untrusted input; the API serves it `Content-Disposition: attachment` so it never renders on the app origin), `redact.ts` (regex scrub of token shapes/emails/home paths, opt-in on both formats). Used by `turnlog export <id> [--html] [--redact]` and `GET /api/sessions/:id/export?format=&redact=`
- `src/mcp/mcp.ts` — `turnlog mcp`: the index as read-only agent memory over a stdio MCP server. Hand-rolled JSON-RPC 2.0 (newline-delimited; no MCP SDK — the zero-dep rule), tools-only surface (`search`, `list_sessions`, `get_session`, `get_messages`, `file_history`) over the same `src/server/api.ts` query functions. In MCP mode stdout is the protocol channel — every diagnostic goes to stderr, never console.log
- `web/` — npm workspace: Vite + React viewer. Visual language: a full-bleed bento system — the background is the app surface, structure comes from rounded card surfaces and spacing (never borders/shadows); tokens live in `web/src/theme.css`, dark + light via `data-theme`. Screens: home (hero + bento), session sidebar zone, replay (turn spine default · log; lenses via `?l=` — the diffs lens renders as the per-file pivot (`replay/Files.tsx`), views via `?v=`, in-session find via `?q=`), search. `web/src/replay/thread.ts` builds display blocks (tool_use/result folding, sidechain runs nested under Task calls); `web/src/replay/raw.ts` re-parses `raw` JSONL tolerantly (UI half of the cardinal rule — degrade, never throw); Shiki runs in a web worker with a lang whitelist + size cap. Fonts are bundled woff2 in `web/public/fonts/`, Solar outline icons vendored as path data in `web/src/icons.tsx` (CC BY 4.0 — credits in `web/public/CREDITS.txt`) — nothing ever loads from the network.
...
Conventions: any adapter change ships with corpus fixtures + regenerated goldens. Bump `ADAPTER_VERSION` in `src/version.ts` when normalization output changes — it forces a full reindex. Every user-visible change gets a line under `[Unreleased]` in `CHANGELOG.md` (Keep a Changelog format); retitle that section to the version when tagging a release. **Before tagging any release, also add that version's entry to `web/src/whatsnew.ts`** — the in-app What's New page (`#/whats-new`, opened via the header status dot) — in user-level language: added / improved / fixed, plain words a user skims, never commit messages. The page ships in the bundle; nothing is fetched. The server-hardening tests in `test/server.test.ts` are load-bearing; never weaken them. Note: `fetch` can't forge a Host header (undici strips it) — hardening tests must use raw `http.request`. This repo is public: keep strategy, planning, and internal notes out of committed files (`CLAUDE.local.md` is the gitignored place for those).
...
- **Threading is a `parentUuid` chain**, not a flat array — branches exist. Sidechain records are subagent runs, rendered as nested collapsible threads. `tool_use` pairs with `tool_result` by ID; results can be enormous — collapse by default. Newer CC also writes Task subagent transcripts to separate files (`<project>/<session>/subagents/*.jsonl`) — indexed as child sessions (`sessions.parent_session_id`), hidden from lists, rolled into the parent's aggregates, and nested in the parent's replay under the Task call that spawned them (anchored by the transcript's opening prompt matching the Task `input.prompt`).
